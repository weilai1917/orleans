...
4.  [Consul IO](https://www.consul.io) - we used [Consul's Key/Value store](https://www.consul.io/intro/getting-started/kv.html) to impelement the membershop table. Refer to [Consul-Deployment](Consul-Deployment.zh.md) for more details.
...
5. `Consul` - configured as Custom system store with `MembershipTableAssembly = "OrleansConsulUtils"`.  Refer to [Consul-Deployment](Consul-Deployment.zh.md) for more details.
