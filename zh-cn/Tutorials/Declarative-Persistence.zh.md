...
如果Grains类型具有状态，则在激活时该状态将从存储中加载，然后`非激活异步`调用，以便您可以确保在初始化Grains时加载了状态。这是Orleans唯一调用来的案子`读状态异步`自动地。如果你想写州或在其他地方读，你应该自己写。通常你不需要调用`读状态异步`你自己，除非你正在做一些关于处理损坏状态或其他事情的具体事情。
