...
本文档解释了如何在应用程序出口之前优雅地关闭奥尔良筒仓，首先是作为控制台应用程序，然后作为一个反编译容器应用程序。

# 正常关机-控制台应用程序

下面的代码显示了如何在用户按下ctrl+c时优雅地关闭Orleans思洛控制台应用程序，这将生成`控制台取消按键`事件

通常，当事件处理程序返回时，应用程序将立即退出，导致灾难性的奥尔良筒仓崩溃和内存状态丢失。但是在下面的示例代码中，我们设置`a.取消=真；`以防止应用程序在Orleans思洛存储器完成正常关闭之前关闭。
